...
from model_optimizer.backends import get_backend_installer
...
def _mount_tensorrt_engines(policy: Any, config: ServerConfig, on_progress: ProgressCallback) -> None:
    ...
    get_backend_installer(config.architecture, "tensorrt").install(policy, config, on_progress)


def _mount_native(policy: Any, config: ServerConfig, on_progress: ProgressCallback) -> None:
    """在已有策略上挂载 Native decoder 路径（expert / denoise）。"""
    get_backend_installer(config.architecture, "native").install(policy, config, on_progress)
...
def _mount_onnxrt_engines(policy: Any, config: ServerConfig, on_progress: ProgressCallback) -> None:
    ...
    get_backend_installer(config.architecture, "onnxrt").install(policy, config, on_progress)
...
def load_policy_for_serve(
    ...
    if config.mode in ("native", "tensorrt", "pt_trt_compare", "ptq_trt_compare"):
        _mount_native(policy, config, on_progress)
